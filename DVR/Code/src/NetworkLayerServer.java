...
public class NetworkLayerServer {
    ...
    public static void main(String[] args) {
        ...
        while(true) {
            try {
                ...
                System.out.println("\nClient" + (clientCount + 1) + " attempted to connect");
    ...
    public static synchronized void DVR(int startingRouterId) {
        ...
        while(convergence) {
            ...
            Router startRouter = NetworkLayerServer.routerMap.get(startingRouterId);
            for(int neighborID: startRouter.getNeighborRouterIDs()) {
                Router neighbour = NetworkLayerServer.routerMap.get(neighborID);
                if(neighbour.getState()) {
                    convergence = neighbour.sfupdateRoutingTable(startRouter);
                }
            }

            for(Router router: NetworkLayerServer.routers) {
                if(router.getRouterId()==startingRouterId) continue;

                for(int neighborID: router.getNeighborRouterIDs()) {
                    Router neighbour = NetworkLayerServer.routerMap.get(neighborID);
                    if(neighbour.getState()) {
                        convergence = neighbour.sfupdateRoutingTable(router);
                    }
                }
            }
        }
        ...
        */
        ...
        while(convergence) {
            ...
            for(int neighborID: startRouter.getNeighborRouterIDs()) {
                Router neighbour = NetworkLayerServer.routerMap.get(neighborID);
                if(neighbour.getState()) {
                    convergence = neighbour.updateRoutingTable(startRouter);
                }
            }
            ...
            for(Router router: NetworkLayerServer.routers) {
                ...
                for(int neighborID: router.getNeighborRouterIDs()) {
                    Router neighbour = NetworkLayerServer.routerMap.get(neighborID);
                    if(neighbour.getState()) {
                        convergence = neighbour.updateRoutingTable(router);
                    }
                }
            }
        }
    ...
    public static EndDevice getClientDeviceSetup() {
        ...
        //System.out.println("Size: " + clientInterfaces.size() + " Random 'r': " + r);
